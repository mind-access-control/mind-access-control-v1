...
const recentAccesses = [
  { id: 1, timestamp: "2025-01-10 14:32", user: "John Smith", status: "Success" },
  { id: 2, timestamp: "2025-01-10 14:28", user: "Sarah Johnson", status: "Success" },
  { id: 3, timestamp: "2025-01-10 14:15", user: "Mike Wilson", status: "Failure" },
  { id: 4, timestamp: "2025-01-10 14:02", user: "Emily Davis", status: "Success" },
  { id: 5, timestamp: "2025-01-10 13:45", user: "Robert Brown", status: "Success" },
]
...
type LogSortField = "timestamp" | "user" | "email" | "role" | "jobTitle" | "zone" | "method" | "status"
...
export default function AdminDashboard({ onLogout }: { onLogout: () => void }) {
  ...
  const [newCamera, setNewCamera] = useState({ name: "", zone: "", location: "" })
  ...
  const toggleAccessZone = (zone: string) => {
    setSelectedAccessZones((prev) => (prev.includes(zone) ? prev.filter((item) => item !== zone) : [...prev, zone]))
  }

  const toggleEditingAccessZone = (zone: string) => {
    setEditingAccessZones((prev) => (prev.includes(zone) ? prev.filter((item) => item !== zone) : [...prev, zone]))
  }
  ...
  const sortedUsers = [...users].sort((a, b) => {
    ...
    if (sortField === "accessZones") {
      aValue = a.accessZones.join(", ")
      bValue = b.accessZones.join(", ")
    }
  ...
  const filteredLogs = useMemo(() => {
    return accessLogs.filter((log) => {
      // Apply all filters
      if (selectedUser && selectedUser !== "all" && log.user !== selectedUser) return false
      if (selectedZone && selectedZone !== "all" && log.zone !== selectedZone) return false
      if (selectedStatus && selectedStatus !== "all" && log.status !== selectedStatus) return false
      if (selectedMethod && selectedMethod !== "all" && log.method !== selectedMethod) return false
  ...
  const userSummaryData = useMemo(() => {
    const summaryData = Array.from(new Set(accessLogs.map((log) => log.user))).map((userName) => {
      const userLogs = accessLogs.filter((log) => log.user === userName)
      const userEmail = userLogs[0]?.email || ""
      const firstAccess = userLogs.reduce((earliest, log) => (log.timestamp < earliest.timestamp ? log : earliest))
      const lastAccess = userLogs.reduce((latest, log) => (log.timestamp > latest.timestamp ? log : latest))

      // Count accesses per zone
      const zoneAccesses = userLogs.reduce(
        (acc, log) => {
    ...
    return userSummaryData
      .filter((summary) => {
        ...
        if (summarySearchTerm) {
          ...
          return summary.user.toLowerCase().includes(searchLower) || summary.email.toLowerCase().includes(searchLower)
        }

        // Apply status filter
        if (summaryStatusFilter === "successful" && summary.successful === 0) return false
        if (summaryStatusFilter === "failed" && summary.failed === 0) return false
      ...
      .sort((a, b) => {
        ...
        // Special case for timestamp sorting
        if (summarySortField === "firstAccess" || summarySortField === "lastAccess") {
  ...
  const saveEditingZone = () => {
    ...
    setZones(zones.map((zone) => (zone.id === editingZoneId ? { ...zone, name: editingZoneName.trim() } : zone)))

    // Also update zone references in cameras
    const oldZoneName = zones.find((z) => z.id === editingZoneId)?.name
    if (oldZoneName) {
      setCameras(
        cameras.map((camera) => (camera.zone === oldZoneName ? { ...camera, zone: editingZoneName.trim() } : camera)),
      )
    }
  ...
  const confirmZoneDelete = () => {
    ...
    setCameras(cameras.map((camera) => (camera.zone === zoneToDelete.name ? { ...camera, zone: "" } : camera)))
        ...
        {activeTab === "dashboard" && (
          <div className="space-y-8">
            <div>
              ...
              <p className="text-indigo-200">Overview of the Facial Access Control System</p>
            </div>
                ...
                <CardContent className="p-6">
                  <div className="flex items-center">
                    ...
                    <div className="ml-4">
                      ...
                      <p className="text-2xl font-bold text-gray-900">{summaryData.totalUsers}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
                ...
                <CardContent className="p-6">
                  <div className="flex items-center">
                    ...
                    <div className="ml-4">
                      ...
                      <p className="text-2xl font-bold text-gray-900">{summaryData.activeZones}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="bg-white shadow-lg">
                <CardContent className="p-6">
                  <div className="flex items-center">
                    ...
                    <div className="ml-4">
                      <p className="text-sm font-medium text-gray-600">Accesses Today</p>
                      <p className="text-2xl font-bold text-gray-900">{summaryData.accessesToday}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
            ...
            <Card className="bg-white shadow-lg">
              ...
              <CardContent>
                {recentAccesses.length > 0 ? (
                        ...
                        <TableRow key={access.id}>
                          ...
                          <TableCell>
                            <Badge
                              ...
                              className={access.status === "Success" ? "bg-green-100 text-green-800" : ""}
                            >
                              {access.status}
                            </Badge>
                          </TableCell>
                  ...
                  <p className="text-gray-500 text-center py-4">No recent access attempts.</p>
                )}
              </CardContent>
            </Card>
          </div>
              ...
              <CardContent className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Select value={selectedJobTitle} onValueChange={setSelectedJobTitle}>
                      <SelectTrigger id="jobTitle" className="bg-slate-50 border-0 h-12">
                  ...
                  {selectedAccessZones.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-2">
                      {selectedAccessZones.map((zone) => (
                        <Badge key={zone} variant="secondary" className="bg-slate-100">
                          {zone}
                          <button className="ml-1 hover:text-red-500" onClick={() => toggleAccessZone(zone)}>
                            ×
                          </button>
                        </Badge>
                      ))}
                ...
                <div className="space-y-4">
                  ...
                  {!imagePreview ? (
                    <div
                      className={`border-2 ${
                        isDragging ? "border-teal-500 bg-teal-50" : "border-dashed border-gray-300"
                    ...
                    <div className="space-y-4">
                      ...
                      <p className="text-xs text-gray-500 italic">
                        You can also drag and drop a new image to replace the current one
                      </p>
                    </div>
                  )}
                  ...
                  <div className="bg-blue-50 p-4 rounded-lg">
                    <p className="text-sm text-blue-800">
                      <strong>Instructions:</strong> Upon saving this user, the system will automatically detect faces
                      in the image and generate a 'facial embedding' for authentication. Ensure the image contains a
                      clear, well-lit face.
                    </p>
                  </div>
                </div>

                <Button className="bg-teal-600 hover:bg-teal-700" onClick={handleSaveUser}>
                  Save User
                </Button>
              </CardContent>
            </Card>
                          ...
                          <TableCell>
                            {editingUserId === user.id ? (
                              <Select
                                ...
                                onValueChange={(value) => updateEditingUser("jobTitle", value)}
                              >
                                <SelectTrigger className="h-8">
                                  <SelectValue />
                                </SelectTrigger>
                                      ...
                                      {accessZonesOptions.map((zone) => (
                                        <div key={zone} className="flex items-center space-x-2">
                                          <Checkbox
                                            ...
                                            checked={editingAccessZones.includes(zone)}
                                            onCheckedChange={() => toggleEditingAccessZone(zone)}
                                          />
                                          <label
            ...
            <Card className="bg-white shadow-lg">
              ...
              <CardContent>
                <p className="text-gray-600 mb-4">
                  Upload a CSV file containing user data for bulk registration. The CSV must include columns for Full
                  Name, Email Address, User Role, Job Title, Access Zones (comma-separated), and a 'Photo URL' where
                  each user's facial recognition image is publicly accessible.
                </p>
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={selectedUser} onValueChange={setSelectedUser}>
                      ...
                      <SelectContent>
                        ...
                        {Array.from(new Set(accessLogs.map((log) => log.user))).map((user) => (
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={selectedZone} onValueChange={setSelectedZone}>
                      ...
                      <SelectContent>
                        ...
                        {Array.from(new Set(accessLogs.map((log) => log.zone))).map((zone) => (
                      ...
                      {paginatedLogs.length > 0 ? (
                        paginatedLogs.map((log) => (
                          <TableRow key={log.id}>
                            ...
                            <TableCell className="text-gray-600">{log.jobTitle}</TableCell>
                            <TableCell>
                ...
                <div className="flex items-center justify-between mt-4 pt-4 border-t">
                  <div className="flex items-center space-x-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) => setLogItemsPerPage(Number(value))}
                    >
                      <SelectTrigger className="w-20 h-8">
                        <SelectValue />
                      </SelectTrigger>
                  ...
                  <div className="flex items-center space-x-2">
                    <span className="text-sm text-gray-600">
                      Showing {logStartIndex + 1} to {Math.min(logEndIndex, sortedLogs.length)} of {sortedLogs.length}{" "}
                      logs
                    </span>
                  </div>
                ...
                <Tabs defaultValue="zones" value={activeSettingsTab} onValueChange={setActiveSettingsTab}>
                  ...
                  <TabsContent value="zones" className="space-y-6">
                    <div>
                      ...
                      <p className="text-gray-600 mb-4">
                        Define and manage access zones for your facility. Each zone can have multiple cameras assigned
                        to it.
                      </p>
                    </div>
                  ...
                  <TabsContent value="cameras" className="space-y-6">
                    <div>
                      ...
                      <p className="text-gray-600 mb-4">
                        Manage cameras and assign them to specific access zones. Each camera can be assigned to one
                        zone.
                      </p>
                    </div>
                                  ...
                                  <TableCell>
                                    {editingCameraId === camera.id ? (
                                      <Select
                                        ...
                                        onValueChange={(value) => setEditingCamera({ ...editingCamera, zone: value })}
                                      >
                                        <SelectTrigger className="h-8">
                                          <SelectValue />
                                        </SelectTrigger>
      ...
      <Dialog open={bulkUploadModalOpen} onOpenChange={setBulkUploadModalOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Upload a CSV file containing user data for bulk registration. The CSV must include columns for Full Name,
              Email Address, User Role, Job Title, Access Zones (comma-separated), and a 'Photo URL'.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4">
            {uploadStatus === "idle" ? (
              <div
                className={`border-2 ${
                  csvIsDragging ? "border-teal-500 bg-teal-50" : "border-dashed border-gray-300"
                } rounded-lg p-6 transition-colors`}
              ...
              >
                <div className="text-center">
                  <FileSpreadsheet
                    className={`mx-auto h-12 w-12 ${csvIsDragging ? "text-teal-500" : "text-gray-400"}`}
                  />
                  <div className="mt-4">
                    <p className="text-sm text-gray-500 mb-2">
            ...
            <div className="flex flex-col gap-2">
              ...
              <p className="text-xs text-gray-500">
                Download a template CSV file with the required headers for bulk upload.
              </p>
            </div>
          </div>
      ...
      <Dialog open={summaryModalOpen} onOpenChange={setSummaryModalOpen}>
        <DialogContent className="sm:max-w-6xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Daily access summary for all users (Today: {new Date().toLocaleDateString()})
            </DialogDescription>
          </DialogHeader>
                ...
                <CardContent className="p-4">
                  <div className="text-center">
                    <p className="text-2xl font-bold text-teal-600">
                      {Array.from(new Set(accessLogs.map((log) => log.user))).length}
                    </p>
                ...
                <CardContent className="p-4">
                  <div className="text-center">
                    <p className="text-2xl font-bold text-green-600">
                      {accessLogs.filter((log) => log.status === "Successful").length}
                    </p>
                ...
                <CardContent className="p-4">
                  <div className="text-center">
                    <p className="text-2xl font-bold text-blue-600">
                      {Array.from(new Set(accessLogs.map((log) => log.zone))).length}
                    </p>
                        ...
                        <TableRow key={summary.user}>
                          ...
                          <TableCell>
                            <div className="flex flex-wrap gap-1">
                              {Object.entries(summary.zoneAccesses).map(([zone, count]) => (
      ...
      <Dialog open={zoneDeleteModalOpen} onOpenChange={setZoneDeleteModalOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>Confirm Delete</DialogTitle>
            <DialogDescription>
              Are you sure you want to delete the zone <strong>{zoneToDelete?.name}</strong>? This action cannot be
              undone.
              {cameras.some((camera) => camera.zone === zoneToDelete?.name) && (
                <div className="mt-2 text-red-600">
                  Warning: This zone has cameras assigned to it. Deleting this zone will unassign these cameras.
                </div>
              )}
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={cameraDeleteModalOpen} onOpenChange={setCameraDeleteModalOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>Confirm Delete</DialogTitle>
            <DialogDescription>
              Are you sure you want to delete the camera <strong>{cameraToDelete?.name}</strong>? This action cannot be
              undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="flex gap-2">
